...
## 9. 图是怎么被释放的（shared_ptr 单向引用）

你可能担心：节点存了输入张量的句柄，这些 shared_ptr 会不会循环引用泄漏？不会——因为**引用永远是"新 → 旧"单向的**：

```
loss ──grad_fn──► 节点 ──► 输入张量 ──grad_fn──► 更早的节点 ──► ...
```

每个张量只被一个节点"生产"（`grad_fn` 指向它），节点又只引用更早的张量。这是一张严格单向的 DAG，**不可能有环**。当最后一个用户句柄析构时，`loss` 释放，触发整条链从新到旧依次释放。

> 对比 PyTorch 需要一套 intrusive_ptr + 版本计数器，是因为它有 view 共享存储、in-place 修改这些更复杂的语义。教学版砍掉这些，shared_ptr 就够用了。
